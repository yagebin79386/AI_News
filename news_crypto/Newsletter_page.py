...
if os.environ.get('RUNNING_IN_PRODUCTION') == 'true':
    app.config['APPLICATION_ROOT'] = '/crypto/newsletter'
    logger.info("Running in production mode with APPLICATION_ROOT set to /crypto/newsletter")
else:
...
def index():
    ...
    <body>
        ...
        <p>Example: <a href="/crypto/newsletter/1">/crypto/newsletter/1</a> to view newsletter #1</p>
    </body>
    </html>
    """
...
def display_newsletter(newsletter_id):
    ...
    try:
        ...
        cursor.execute("SELECT html FROM crypto_newsletter WHERE newsletter_id = %s", (newsletter_id,))
